	...
	Properties
	{
	  ...
	  [HideInInspector]_Distance8("Distance8", float) = 0
		...
		SubShader
		{
			...
			CGPROGRAM
			#pragma surface surf Lambert vertex:vert noambient noshadow
			...
			void vert(inout appdata_full v, out Input o) {
				...
				half value = _Scale * sin(_Time.w * _Speed + offsetvert2 * _Frequency);
				...
				if (sqrt(pow(worldPos.x - _xImpact1, 2) + pow(worldPos.z - _zImpact1, 2)) < _Distance1)
				{
					...
					//v.vertex.y += value1 * (_WaveAmplitude1+10);
					//v.normal.y += value1 * (_WaveAmplitude1 + 10);
					//o.customValue += value1 * (_WaveAmplitude1 + 10);
			...
			void surf(Input IN, inout SurfaceOutput o)
			{
				...
				float4 nor1 = tex2D(_BumpMap, IN.uv_BumpMap + float2(_WaveSpeed, 0));
				float4 nor2 = tex2D(_BumpMap, IN.uv_BumpMap - float2(_WaveSpeed, 0));
